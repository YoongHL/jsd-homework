...
function handlebyClass() {
  let div = document.querySelector('div')
  div.innerHTML = ''
  let classValue = select.value  

  glossary
    .filter((item) => item.class === parseInt(classValue))
    .forEach((item) => {
      let p = document.createElement('p')
...
function handlebyTags() {
  ...
  for (let i = 0; i <= checkbox.length -1; i++) {
    if (checkbox[i].checked === true) {
      tagsValue = checkbox[i].value       
    } else {
      tagsValue = ""      
    }

    if (tagsValue !== "") {      
      glossary.filter((item) => item.tags.includes(tagsValue) === true)
      .forEach((item) => {
        let p = document.createElement('p')
        p.innerHTML = `${item.definition}`
        div.appendChild(p)
...
function handlebyAll() {
  ...
  let classValue = select.value
  let tagsValue;
  let finalData = glossary;

  //Keyword
  if(term !== "") {
    finalData = finalData.filter((item) => item.term === term)
  }

  //Class
  finalData = finalData.filter((item) => item.class === parseInt(classValue))
  ...
  for (let i = 0; i <= checkbox.length -1; i++) {
    ...
    if (tagsValue !== "") {      
      finalData = finalData.filter((item) => item.tags.includes(tagsValue) === true)      
    }
  }
...
button[0].addEventListener('click', handleOnChange)
button[1].addEventListener('click', handlebyClass)
button[2].addEventListener('click', handlebyTags)
button[3].addEventListener('click', handlebyAll)